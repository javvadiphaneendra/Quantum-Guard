...
def generate_bits_and_bases(n=20):
    bits = [random.randint(0, 1) for _ in range(n)]
    bases = [random.choice(["+", "x"]) for _ in range(n)]  # + = rectilinear, x = diagonal
    return bits, bases
...
def eve_attack(bits, bases):
    ...
    for bit, base, eve_base in zip(bits, bases, eve_bases):
        if base == eve_base:
            ...
            eve_measurements.append(random.randint(0, 1))  # Wrong measurement
...
def animate_bb84(alice_bits, alice_bases, bob_clean_bits, bob_clean_bases, bob_eve_bits, bob_eve_bases, sifted_clean, qber_clean, sifted_eve, qber_eve):
    ...
    fig.suptitle("QUANTUM GAURD(BB84 Quantum Key Distribution) — Clean vs Attacked ")

    # CLEAN channel plots
    ax1, ax2 = axes[0]
    ax1.set_title("Alice → Bob (Clean Channel)")
    ...
    # ATTACKED channel plots
    ax3, ax4 = axes[1]
    ax3.set_title("Alice → Eve → Bob (Attacked Channel)")
...
if __name__ == "__main__":
    ...
    # Connect with Bob (clean case)
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.connect((HOST, PORT))
        msg = {"alice_bits": alice_bits, "alice_bases": alice_bases}
        s.sendall(json.dumps(msg).encode())
        data = s.recv(4096).decode()
        bob_data = json.loads(data)
        bob_clean_bits = bob_data["bob_bits"]
        bob_clean_bases = bob_data["bob_bases"]

    # Case 1: Clean
    sifted_clean, qber_clean, _ = sift_key(alice_bits, alice_bases, bob_clean_bits, bob_clean_bases)

    # Case 2: Eve intercepts
    eve_measurements, eve_bases = eve_attack(alice_bits, alice_bases)
    bob_eve_bits = eve_measurements  # Bob gets Eve's forwarded bits
    bob_eve_bases = eve_bases
    sifted_eve, qber_eve, _ = sift_key(alice_bits, alice_bases, bob_eve_bits, bob_eve_bases)

    print(f"[Alice] Clean QBER: {qber_clean*100:.1f}% | Attacked QBER: {qber_eve*100:.1f}%")

    animate_bb84(alice_bits, alice_bases, bob_clean_bits, bob_clean_bases, bob_eve_bits, bob_eve_bases, sifted_clean, qber_clean, sifted_eve, qber_eve)
